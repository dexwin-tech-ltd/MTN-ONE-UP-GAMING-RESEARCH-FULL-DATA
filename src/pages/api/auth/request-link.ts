...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (!emailAllowed || !ipAllowed) {
      return Response.redirect(
        new URL("/auth?status=rate-limited", request.url),
        303,
      )
    }
  } catch {
  ...
  if (!isAllowedEmailDomain(email)) {
    return Response.redirect(
      new URL("/auth?status=domain-error", request.url),
      303,
    )
  }
  ...
  return Response.redirect(
    new URL("/auth?status=check-email", request.url),
    303,
  )
}

...
export async function enforceRateLimit(locals: App.Locals, input: RateLimitInput): Promise<boolean> {
  ...
  if (!existing) {
    await db
      .prepare('INSERT INTO auth_rate_limits (key, bucket, window_start, count) VALUES (?, ?, ?, 1)')
